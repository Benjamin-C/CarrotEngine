...
public class TracedPrintStream extends PrintStream{
	...
	private int defStep = 2;
	private String stackTrace(int rel, int step) {
		StackTraceElement loc = Thread.currentThread().getStackTrace()[defStep + step];
		if(fullStackTrace) {
			String st = "";
			for(int i = Thread.currentThread().getStackTrace().length - 1; i > step + defStep; i--) {
